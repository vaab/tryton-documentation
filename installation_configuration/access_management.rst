...
#. If contradictory access rules apply to a single user, the most permissive applies.
#. For models/fields which have no access rules, everyone by default has access.  But once a model/field has an associated access rule, the default rule becomes no-access.  The existence of an access rule implicitly restricts users not explicitly given access.
